...
class AlbaInterestPayoutWizard(models.TransientModel):
    ...
    def _get_accrual_cutoff_breakdown(self, accrual):
        ...
        Fix B — Double-payment on an in-progress (still-open) period:
        interest_payout_id/interest_amount_deferred only track a genuine residual
        once the period is COMPLETE (rule 1a below) — for an in-progress period,
        every payout recomputed the cutoff split from scratch with no memory of
        what was already paid, so a second "pay outstanding" click on the same
        open period paid the same days' interest again. cumulative_amount_paid
        is a separate running total, updated on every payout regardless of
        whether the period was open or complete at the time, and is subtracted
        here in both branches.
        ...
        1. Period complete (period_end <= payout_date):
           ...
           b. Otherwise: full interest_amount minus whatever has already been paid
              (cumulative_amount_paid) — covers both a fresh accrual (0 paid) and one
              that received one or more payouts while its period was still open.
        2. Period still in progress: split by the 15th-of-month cutoff, then subtract
           whatever has already been paid toward this same open period.
        """
        payout_date = self.payout_date or fields.Date.today()
        already_paid = accrual.cumulative_amount_paid or 0.0
        ...
        if accrual.period_end and accrual.period_end <= payout_date:
            ...
            remaining = (accrual.interest_amount or 0.0) - already_paid
            return max(remaining, 0.0), 0.0

        # In-progress period — split by the 15th-of-month payout cutoff, then
        # subtract anything already paid toward this period while it was open.
        eligible, deferred = split_period_for_payout_cutoff(
            accrual.period_start,
            accrual.period_end,
            accrual.interest_amount,
        )
        payable_now = max((eligible or 0.0) - already_paid, 0.0)
        return payable_now, deferred
    ...
    def action_confirm_payout(self):
        ...
        # near-simultaneous payout confirmations (double-click, two tabs)
        # can't both read the same "posted"/outstanding state before either
        # commits. The second confirm blocks here until the first commits.
        self.env.cr.execute(
        ...
        # ── Validate oldest-first ordering (Select Specific Months mode) ───────
        # Step 6 below deletes any OTHER posted accrual dated after the
        # earliest month actually paid in this payout — because paying a
        # month removes its interest from the compounding base, every later
        # month's stored opening_balance/interest_amount is now genuinely
        # stale and must be regenerated by the cron. That's only safe to do
        # silently when the payout itself was oldest-first; paying a later
        ...
        if self.payout_mode == "select" and self.selected_accrual_ids:
            ...
            if selected:
                ...
                earlier_unselected = investment.accrual_ids.filtered(
                    lambda a: a.state == "posted"
                    and a.id not in selected.ids
                    and a.period_start < earliest_selected_start
                )
                if earlier_unselected:
                    raise UserError(_(
                        "Cannot pay %(count)d selected month(s) while an earlier "
                        "unpaid accrual (period starting %(date)s) is left out — "
                        "paying out of order would delete later, still-unpaid "
                        "accrual records so the cron can regenerate them with a "
                        "correct opening balance. Please pay accruals oldest-first: "
                        "include %(date)s in this payout, or pay it separately first.",
                        count=len(selected),
                        date=min(earlier_unselected.mapped("period_start")),
                    ))
        ...
        if self.payout_mode in ("all", "select"):
            for accrual in accrued_candidates:
                ...
                accrual.write({
                    ...
                    "interest_amount_deferred": deferred_amount,
                    "cumulative_amount_paid": (accrual.cumulative_amount_paid or 0.0) + payable_now,
                })
            ...
            for accrual in accrued_candidates.sorted("accrual_date"):
                ...
                if remaining >= payable_now:
                    ...
                    accrual.write({
                        ...
                        "cumulative_amount_paid": (accrual.cumulative_amount_paid or 0.0) + remaining,
                    })
