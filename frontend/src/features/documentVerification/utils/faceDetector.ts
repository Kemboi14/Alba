 ...
 */
export async function detectFace(imageFile: File): Promise<FaceDetectionResult> {
  ...
  try {
    ...
    if (detections.length === 0) {
      return {
        ...
        boundingBox: null,
        confidence: 0,
        warnings: ['No face detected. Please ensure your face is clearly visible.'],
      };
    }

    // Multiple faces detected
    if (detections.length > 1) {
      warnings.push('Multiple faces detected. Please ensure only your face is in the frame.');
    ...
    if (faceRatio < 0.1) {
      warnings.push('Face appears too small in the image. Please move closer to the camera.');
    }
    
    if (faceRatio > 0.8) {
      warnings.push('Face appears too large or too close. Please move back slightly.');
    }
    ...
    if (offsetX > 0.3 || offsetY > 0.3) {
      warnings.push('Face is not centered. Please position your face in the center of the frame.');
    }

    // Check detection confidence
    const confidence = Math.round(detection.detection?.score * 100 || 0);
    if (confidence < 70) {
      warnings.push('Face detection confidence is low. Please ensure good lighting and clear view.');
    }
    ...
    return {
      ...
      boundingBox: null,
      confidence: 0,
      warnings: ['Failed to process image. Please try again with a clearer photo.'],
    };
  }
}
...
export function getFaceGuidance(result: FaceDetectionResult): string {
  ...
  if (result.quality === 'poor') {
    const warning = result.warnings[0] || 'Adjust your position for better detection';
    return warning;
  }
...
export async function checkCameraSupport(): Promise<{
  ...
  try {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      return { supported: false, error: 'Camera API not supported in this browser' };
    }
