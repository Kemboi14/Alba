...
class LoanProduct(models.Model):
    ...
    category = models.CharField('Category', max_length=20, choices=PRODUCT_CATEGORY_CHOICES)
    ...
    # Repayment Terms
    min_tenure_months = models.PositiveIntegerField('Minimum Tenure (Months)', default=1)
    max_tenure_months = models.PositiveIntegerField('Maximum Tenure (Months)', default=12)
    ...
    requires_employer_verification = models.BooleanField('Requires Employer Verification', default=False)
...
class Customer(models.Model):
    ...
    id_number = models.CharField('ID/Passport Number', max_length=50, unique=True, null=True, blank=True)
...
class CreditScore(models.Model):
    ...
    income_score = models.DecimalField('Income Score', max_digits=5, decimal_places=2, default=0)
    employment_score = models.DecimalField('Employment Score', max_digits=5, decimal_places=2, default=0)
    credit_history_score = models.DecimalField('Credit History Score', max_digits=5, decimal_places=2, default=0)
    existing_obligations_score = models.DecimalField('Existing Obligations Score', max_digits=5, decimal_places=2, default=0)
    age_score = models.DecimalField('Age Score', max_digits=5, decimal_places=2, default=0)
    ...
    # Calculation Details
    calculation_details = models.JSONField('Calculation Details', default=dict, blank=True)
...
class LoanApplication(models.Model):
    ...
    repayment_frequency = models.CharField(
        'Repayment Frequency',
        max_length=15,
        choices=LoanProduct.FREQUENCY_CHOICES
    )
    ...
    def can_transition_to(self, new_status):
        ...
        valid_transitions = {
            ...
            self.EMPLOYER_VERIFICATION: [self.GUARANTOR_CONFIRMATION, self.DISBURSED, self.REJECTED],
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.loan_number:
            ...
            last_loan = Loan.objects.filter(
                loan_number__startswith=f'LN-{date_str}'
...
class LoanRepayment(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.receipt_number:
            from django.utils import timezone
            date_str = timezone.now().strftime('%Y%m%d')
            last_receipt = LoanRepayment.objects.filter(
...
class EmployerVerification(models.Model):
    ...
    sent_at = models.DateTimeField('Verification Request Sent At', null=True, blank=True)
...
class GuarantorVerification(models.Model):
    ...
    sent_at = models.DateTimeField('Confirmation Request Sent At', null=True, blank=True)
...
class LoanDocument(models.Model):
    ...
    document_file = models.FileField(
        'Document File',
        upload_to='loan_documents/%Y/%m/%d/'
    )
